...
# ============================================================================
# Embeddings (OpenAI-compatible) - required by core.memory
# ============================================================================
def embed_texts(
    texts,
    model=None,
    ...
    """
    Zwraca listę embeddingów (list[float]) dla listy tekstów.

    Konfiguracja (ENV):
      - EMBED_BASE_URL / EMBED_API_KEY / EMBED_MODEL
      - fallback: LLM_BASE_URL / LLM_API_KEY / LLM_EMBED_MODEL
    Endpoint:
      - {BASE_URL}/embeddings
    """
    import os
    ...
    if isinstance(texts, (str, bytes)):
        texts = [texts.decode() if isinstance(texts, bytes) else texts]
    texts = [("" if t is None else str(t)) for t in list(texts)]

    env_base = (os.getenv("EMBED_BASE_URL") or os.getenv("LLM_BASE_URL") or "").strip()
    env_key = (os.getenv("EMBED_API_KEY") or os.getenv("LLM_API_KEY") or "").strip()
    env_model = (os.getenv("EMBED_MODEL") or os.getenv("LLM_EMBED_MODEL") or "").strip()

    base_url = (base_url or env_base).strip().rstrip("/")
    api_key = (api_key or env_key).strip()
    model = (model or env_model).strip()

    if not base_url:
        raise RuntimeError("Brak EMBED_BASE_URL/LLM_BASE_URL - nie mam gdzie wysłać /embeddings")
    if not api_key:
        raise RuntimeError("Brak EMBED_API_KEY/LLM_API_KEY - embeddings wymagają klucza")
    if not model:
    ...
    for attempt in range(1, int(max_retries) + 1):
        try:
            ...
            if r.status_code != 200:
                raise RuntimeError(f"/embeddings HTTP {r.status_code}: {r.text[:400]}")
            data = r.json()
            ...
            for it in items:
                ...
                if not isinstance(emb, list) or not emb:
                    raise RuntimeError("Nieprawidłowa odpowiedź embeddings (brak embedding[])")
                out.append([float(x) for x in emb])

            if len(out) != len(texts):
                raise RuntimeError(f"Embeddings size mismatch: got {len(out)} expected {len(texts)}")
            return out
        except Exception as e:
            last_err = e
