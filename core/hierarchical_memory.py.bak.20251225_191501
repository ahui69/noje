...
# =====================================================================
# BACKWARDS-COMPAT: globalny dostęp do hierarchicznej pamięci
# =====================================================================
from typing import Optional

try:  # jeśli już istnieje, zostawiamy
    _GLOBAL_HIER_MEMORY  # type: ignore[name-defined]
except NameError:
    _GLOBAL_HIER_MEMORY: Optional["HierarchicalMemorySystem"] = None  # type: ignore[name-defined]


def get_hierarchical_memory() -> "HierarchicalMemorySystem":  # type: ignore[name-defined]
    """
    Shim dla starych modułów (inner_language, future_predictor, self_reflection, itp.).
    Zwraca globalną instancję HierarchicalMemorySystem.
    """
    global _GLOBAL_HIER_MEMORY

    if _GLOBAL_HIER_MEMORY is not None:
        return _GLOBAL_HIER_MEMORY

    for factory_name in (
        "init_hierarchical_memory",
        "create_hierarchical_memory",
        "build_hierarchical_memory",
    ):
        factory = globals().get(factory_name)
        if callable(factory):
            _GLOBAL_HIER_MEMORY = factory()  # type: ignore[misc]
            break

    if _GLOBAL_HIER_MEMORY is None:
        cls = globals().get("HierarchicalMemorySystem")
        if cls is None:
            raise RuntimeError(
                "HierarchicalMemorySystem not defined w core.hierarchical_memory."
            )
        _GLOBAL_HIER_MEMORY = cls()  # type: ignore[misc]

    return _GLOBAL_HIER_MEMORY


# alias klasy dla testów: tests.test_hierarchical_memory importuje HierarchicalMemory
if "HierarchicalMemory" not in globals() and "HierarchicalMemorySystem" in globals():
    class HierarchicalMemory(globals()["HierarchicalMemorySystem"]):  # type: ignore[misc]
        """Alias kompatybilny wstecznie."""
        pass
