...
# ----------------------------
# Compatibility: verify_token()
# ----------------------------
def verify_token(token_or_auth_header, expected_token: str | None = None) -> bool:
    """
    Verify bearer token against AUTH_TOKEN from environment (or provided expected_token).
    Accepts either:
      - raw token string
    ...
    expected = (expected_token or os.getenv("AUTH_TOKEN") or "").strip()
    if not expected:
        raise ValueError("AUTH_TOKEN is not set (cannot verify token).")

    raw = (token_or_auth_header or "").strip()
    if not raw:
